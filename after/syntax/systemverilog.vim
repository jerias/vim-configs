...
" verilogGlobal (backtick macros, `ifdef etc.): colour set by s:base16_customize().
" Fallback to PreProc if colorscheme customization hasn't run.
hi def link verilogGlobal      PreProc

" verilogBlockDelim: colour + bold set by s:base16_customize() in extended.vim.
" Fallback link in case colorscheme customization hasn't run yet.
hi def link verilogBlockDelim  Function
